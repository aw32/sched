...
if __name__ == "__main__":
	...
	if "SCHED_TASKSIZE" not in os.environ or "SCHED_SOCKET" not in os.environ:
		printHelp()
		sys.exit(1)

	socketpath = os.environ["SCHED_SOCKET"]
	tasksize = int(os.environ["SCHED_TASKSIZE"])

	task = CTask(("sleep",5,tasksize,1,5))
...
#		tasks += [CTask(valsn)]
	...
	client = UnixClient(socketpath, [task])
