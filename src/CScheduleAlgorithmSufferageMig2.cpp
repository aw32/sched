...
CScheduleAlgorithmSufferageMig2::CScheduleAlgorithmSufferageMig2(std::vector<CResource*>& rResources)
	: CScheduleAlgorithm(rResources)
{
	mpEstimation = CEstimation::getEstimation();
}

CScheduleAlgorithmSufferageMig2::~CScheduleAlgorithmSufferageMig2() {
	delete mpEstimation;
	mpEstimation = 0;
}

int CScheduleAlgorithmSufferageMig2::init() {
	return 0;
}

void CScheduleAlgorithmSufferageMig2::fini() {
}

CSchedule* CScheduleAlgorithmSufferageMig2::compute(std::vector<CTaskCopy>* pTasks, std::vector<CTaskCopy*>* runningTasks, volatile int* interrupt, int updated) {
	...
	while (unmappedTasks > 0) {
		...
		for (int tix = 0; tix < tasks; tix++) {
			...
			for (int mix = 0; mix<machines; mix++) {
				...
				double readyTask = sched->taskReadyTime(tix);
				double ready = sched->resourceReadyTime(mix);
				if (readyTask > ready) {
					ready = readyTask;
				}
					CLogger::mainlog->debug("Sufferage ready mix %d mixtasks %d tix %d ready %lf",
