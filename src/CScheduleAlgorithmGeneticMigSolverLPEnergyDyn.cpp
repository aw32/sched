...
Dynamic energy per part
	de_1a
*/
void CScheduleAlgorithmGeneticMigSolverLPEnergyDyn::solve(CScheduleAlgorithmGeneticMigDyn::CGeneticSchedule* schedule,
	...
	std::vector<CScheduleAlgorithmGeneticMigSolverLPEnergyDyn::Task> tasklist(pTasks->size());
	...
	for (unsigned int mix=0; mix<rResources.size(); mix++) {
		for (int rix=0; rix<schedule->num[mix]; rix++) {
			...
			int part = schedule->get(mix, rix)->part;
				CLogger::mainlog->debug("Solver: find tix %d part %d mix %d rix %d\n", tix, part, mix, rix);
			if (part == 0) {
	...
	for (unsigned int mix=0; mix<rResources.size(); mix++) {
		...
		for (int rix=0; rix<schedule->num[mix]; rix++) {
			...
			// check if task is already running and this is slot 0
			int run_mix = findTask(runningTasks, task->mId);
			double run_fini = 0.0;
			if (run_mix != -1) {
				run_fini = est->taskTimeFini(task, rResources[run_mix]);
			}
			if (run_mix == (long) mix && rix == 0) {
				init = 0.0;
			}
			...
			if (tasklist[tix].mix_a == tasklist[tix].mix_b &&
				...
				// simple case task is already running on a different machine
				if (run_mix != -1 && run_mix != (long) mix) {
					// FINI <= rm_x_a;
					CLogger::mainlog->debug("LP %lf == rm_%d_a;",
						(run_fini), tix);
					lpfile << (run_fini) << " <= " << "rm_" << tix << "_a;\n";
					// FINI <= rd_x_a;
					CLogger::mainlog->debug("LP %lf == rd_%d_a;",
						(run_fini), tix);
					lpfile << (run_fini) << " <= " << "rd_" << tix << "_a;\n";
				}
			...
			if (part == 0) {
				...
				CLogger::mainlog->debug("GeneticSolver: def durb tix %d",tix);
			...
			if (rix>0 && part == 0) {
				...
				// task is already running on a different machine
				if (run_mix != -1 && run_mix != (long) mix) {
					// FINI - M * bi_x_a <= rm_x_a;
					CLogger::mainlog->debug("LP %lf - lp_max == rm_%d_a;",
						(run_fini), tix);
					lpfile << (run_fini) << " - " << lp_max << " <= " << "rm_" << tix << "_a;\n";
					// FINI <= rd_x_a;
					CLogger::mainlog->debug("LP %lf == rd_%d_a;",
						(run_fini), tix);
					lpfile << (run_fini) << " <= " << "rd_" << tix << "_a;\n";
...
int CScheduleAlgorithmGeneticMigSolverLPEnergyDyn::findTask(std::vector<CTaskCopy*>* tasks, int tid) {
