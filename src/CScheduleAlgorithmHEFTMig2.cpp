...
CScheduleAlgorithmHEFTMig2::CScheduleAlgorithmHEFTMig2(std::vector<CResource*>& rResources)
	: CScheduleAlgorithm(rResources)
{
	mpEstimation = CEstimation::getEstimation();
}

CScheduleAlgorithmHEFTMig2::~CScheduleAlgorithmHEFTMig2() {
	delete mpEstimation;
	mpEstimation = 0;
}

int CScheduleAlgorithmHEFTMig2::init() {
	return 0;
}

void CScheduleAlgorithmHEFTMig2::fini() {
}

CSchedule* CScheduleAlgorithmHEFTMig2::compute(std::vector<CTaskCopy>* pTasks, std::vector<CTaskCopy*>* runningTasks, volatile int* interrupt, int updated) {
	...
	while (listTasks > 0) {
		...
		for (int mixA = 0; mixA < machines; mixA++) {
			...
			CLogger::mainlog->debug("HEFT2Mig: mixA %d durOneA %lf durA %lf", mixA, durOneA, durA);
