...
CScheduleAlgorithmSimulatedAnnealingMigDyn::CScheduleAlgorithmSimulatedAnnealingMigDyn(std::vector<CResource*>& rResources, enum EGeneticFitnessType fitness)
	: CScheduleAlgorithmGeneticMigDyn(rResources, fitness)
{
...
CScheduleAlgorithmSimulatedAnnealingMigDyn::~CScheduleAlgorithmSimulatedAnnealingMigDyn(){
}

int CScheduleAlgorithmSimulatedAnnealingMigDyn::init() {
...
void CScheduleAlgorithmSimulatedAnnealingMigDyn::fini() {
	CScheduleAlgorithmGeneticMigDyn::fini();
}

void CScheduleAlgorithmSimulatedAnnealingMigDyn::randomize(CGeneticSchedule* schedule, int* height, int machines, int height_num, std::vector<int>* height_sets, int max_num){

	// single mutate
	int tries = 0;
	bool mok = false;
	do {
		mok = mutate(schedule, height, machines, height_num, height_sets, max_num);
		tries++;
	} while(mok == false && tries < 5);

}

CSchedule* CScheduleAlgorithmSimulatedAnnealingMigDyn::compute(std::vector<CTaskCopy>* pTasks, std::vector<CTaskCopy*>* runningTasks, volatile int* interrupt, int updated) {
...
*/
	...
	CGeneticSchedule* initialSolution = generateSchedule(height_sets, height_num, machines, max_num*2, pTasks);


	double double_max = 1000000000.0; // punishment value for invalid schedules for the initial run

	CGeneticSchedule* solution = new CGeneticSchedule(max_num*2, machines);
	...
	CGeneticSchedule* bestSolution = new CGeneticSchedule(max_num*2, machines);
	solution->transfer(bestSolution);

	CGeneticSchedule* neighbor = new CGeneticSchedule(max_num*2, machines);


	// initial run to compute temperature for initial probability
	int initialRun_num = loops_num;
	...
	for (int loopIx = 0; loopIx < initialRun_num; loopIx++) {
		...
		for (int i=0; i<1; i++) {
			randomize(neighbor, height, machines, height_num, height_sets, max_num*2);
		}
	...
	if (interrupted == false) {
		...
		for (int mix=0; mix<machines; mix++) {
			for (int rix=0; rix<bestSolution->num[mix]; rix++) {
				...
				int tix = bestSolution->get(mix, rix)->taskid;
				int part = bestSolution->get(mix, rix)->part;
				double ratio = bestSolution->get(mix, rix)->ratio;
				bestSolution->printTask(mix, rix);
				if (ratio == 0.0) {
